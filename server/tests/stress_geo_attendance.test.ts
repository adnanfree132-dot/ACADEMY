...
import {
  ...
  staffCheckInSchema,
  staffCheckOutSchema
} from '../src/validations/staffAttendanceValidation';
import { staffAttendanceStatusEnum, TIME_REGEX } from '../src/validations/commonValidation';
...
async function main() {
  ...
  await runTest('GeoMath', '0m distance on exact coordinate identity (Main Campus 31.5204, 74.3587)', () => {
  ...
  await runTest('GeoMath', 'Antipodal points: North Pole (90, 0) to South Pole (-90, 0) -> ~20,015.09 km', () => {
    const dist = calculateDistance(90, 0, -90, 0);
  ...
  await runTest('GeoMath', 'Antipodal points: Equator (0, 0) to Equator opposite (0, 180) -> ~20,015.09 km', () => {
  ...
  await runTest('GeoMath', 'Antimeridian identity: (0, -180) to (0, 180) represents same 180° meridian -> 0.0m', () => {
    const dist = calculateDistance(0, -180, 0, 180);
    assert(dist === 0.0, `Expected 0.0m on antimeridian identity, got ${dist}m`);
  });

  await runTest('GeoMath', 'Arbitrary antipodes: (45, 90) to (-45, -90) -> ~20,015.09 km', () => {
  ...
  await runTest('GeoMath', 'Coordinate boundary: lat 90.000001 is rejected out of bounds', () => {
  ...
  await runTest('GeoMath', 'Coordinate boundary: lng 180.000001 is rejected out of bounds', () => {
  ...
  await runTest('GeoMath', 'Radius border: point exactly on 150m boundary isInside === true (excess 0m)', () => {
  ...
  await runTest('GeoMath', 'Radius border: point at 150.05m is rejected isInside === false (excess > 0)', () => {
  ...
  await runTest('ShiftLogic', '1 second past grace (08:15:01 vs shift 08:00, grace 15) -> minute-level quantization behavior', () => {
    const status = evaluateShiftStatus('08:15:01', '08:00', 15);
    // Under minute-level resolution: 08:15:01 parses to 495 mins -> present
    assert(status === 'present', `Expected 'present' for 08:15:01 under minute resolution, got '${status}'`);
  });
  ...
  await runTest('ShiftLogic', 'Time parser robustness: parseTimeToMinutes handles boundary and invalid values', () => {
    assert(parseTimeToMinutes('00:00') === 0, '00:00 should be 0 mins');
    assert(parseTimeToMinutes('23:59') === 1439, '23:59 should be 1439 mins');
    assert(parseTimeToMinutes('12:00 AM') === 0, '12:00 AM should be 0 mins');
    assert(parseTimeToMinutes('12:00 PM') === 720, '12:00 PM should be 720 mins');

    let threw25 = false;
    try { parseTimeToMinutes('25:00'); } catch { threw25 = true; }
    assert(threw25, '25:00 must throw in parseTimeToMinutes');

    let threw60 = false;
    try { parseTimeToMinutes('08:60'); } catch { threw60 = true; }
    assert(threw60, '08:60 must throw in parseTimeToMinutes');
  });
  ...
  await runTest('OverrideFuzz', 'Rejects non-padded single-digit time format in check_in_time ("8:00", "8:30")', () => {
    const res = adminAttendanceOverrideSchema.safeParse({
      staff_member_id: 'FAC-2026-001',
      date: '2026-08-25',
      status: 'present',
      check_in_time: '8:00',
      override_reason: 'Invalid time format test'
    });
    assert(res.success === false, 'Non-padded single digit time must be rejected by TIME_REGEX');
  });
