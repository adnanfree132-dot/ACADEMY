...
export const AddTeacherModal: React.FC<AddTeacherModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const local = localStorage.getItem('customTeacherFields');
      if (local) {
        try {
      ...
      api.getSettings().then(settings => {
        if (settings?.customTeacherFields) {
          try {
            const defs = typeof settings.customTeacherFields === 'string' 
              ? JSON.parse(settings.customTeacherFields) 
              : settings.customTeacherFields;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAddTeacher({
      ...
      qualification,
      phone: teacherPhone,
      email: teacherEmail || `${teacherName.toLowerCase().replace(/\s+/g, '.')}@academy.com`,
      ...
      >
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h3 style={{ fontSize: 16, fontWeight: 800, color: '#FFFFFF', margin: 0 }}>Add Faculty Member</h3>
              <p style={{ fontSize: 11, color: '#94A3B8', margin: 0, marginTop: 2 }}>Faculty profile, credentials, and academic qualifications</p>
            </div>
          </div>
          <button 
            type="button"
        ...
        <div style={{ 
          ...
          <form id="add-teacher-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: '#F8FAFC', padding: 16, borderRadius: 14, border: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#2563EB', letterSpacing: '0.05em' }}>PERSONAL & CONTACT DETAILS</div>
            ...
            {/* Card 2: Academic Specialization */}
            <div style={{ background: '#F8FAFC', padding: 16, borderRadius: 14, border: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#059669', letterSpacing: '0.05em' }}>ACADEMIC QUALIFICATION</div>
              
              <div className="form-group">
                <label className="form-label" style={{ fontSize: 12 }}>Qualification / Specialization</label>
                <div className="input-with-icon">
